...
// custom function
		...
		//eg.decrypt(wire.message)
		// remove msg variable

        findElementsByClass(document.getElementById('messageDisplay'), 'data')[0].innerHTML = "Bob's decrypted message: " + stream1;
...
(function () {
    ...
    bind(sender.encryptMsgBtn, 'click', function (e) {
	  ...
	  function dec(s)
	  {
		...
		for(i=0; i<s.length; i++)
          {
			  dummy[i]=String.fromCharCode(buffer[i]);
			  str=str+dummy[i];
		  }
		decry(str);
		}
        ...
        var errorFields = findElementsByClass(sender.el, 'error'), i = 0,
            message = (document.getElementById( sender.messageFieldId ).getElementsByTagName('input')[0].value),
			...
			{
			var converted = stringToAscii(message);
